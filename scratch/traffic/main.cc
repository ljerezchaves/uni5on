...
main (int argc, char *argv[])
{
  ...
  // Create and name the switch node.
  Ptr<Node> switchNode = CreateObject<Node> ();
  Names::Add ("sw", switchNode);

  // Configure switch node as standard OpenFlow switches.
  of13Helper->SetDeviceAttribute ("PipelineTables", UintegerValue (3));
  Ptr<OFSwitch13Device> switchDevice = of13Helper->InstallSwitch (switchNode);
  controllerApp->NotifySwitch (switchDevice);
  ...
  uint32_t sw2svPort = switchDevice->AddSwitchPort (sw2svLink.Get (0))->GetPortNo ();
  NetDeviceContainer serverDevice (sw2svLink.Get (1));

  // Assign IP to the server node and notify the controller.
  Ipv4InterfaceContainer serverIpIface = ipv4helpr.Assign (serverDevice);
  controllerApp->NotifyServer (sw2svPort, serverIpIface.GetAddress (0));

  NetDeviceContainer clientDevices;
  Ipv4InterfaceContainer clientIpIfaces;
  for (uint32_t i = 0; i < numHosts; i++)
    {
      // Connect each client node to the OpenFlow switch.
      NetDeviceContainer sw2clLink = csmaHelper.Install (switchNode, clientNodes.Get (i));
      uint32_t sw2clPort = switchDevice->AddSwitchPort (sw2clLink.Get (0))->GetPortNo ();
      clientDevices.Add (sw2clLink.Get (1));

      // Assign IP to the client node and notify the controller.
      Ipv4InterfaceContainer clIpIface = ipv4helpr.Assign (sw2clLink.Get (1));
      clientIpIfaces.Add (clIpIface);
      controllerApp->NotifyClient (sw2clPort, clIpIface.GetAddress (0));
    }

  // Configure the OpenFlow channel.
  ...
  if (pcap)
    {
      ...
      NodeContainer switchCont (switchNode);
      csmaHelper.EnablePcap (outPrefix + "switch", switchCont,    true);
    }
